...
def analyze_sales_performance():
    ...
    """
    tot_sales = sales_df["Sales"].sum()
    tot_profit = sales_df["Profit"].sum()
    ...
    print("total sales:", round(tot_sales, 2))
    print("total profit:", round(tot_profit, 2))
    print("avg profit margin:", round(avg_margin, 4))

    return {
        "total_sales": float(tot_sales),
        "total_profit": float(tot_profit),
...
def visualize_sales_distribution():
    ...
    """
    by_store = sales_df.groupby("Store")["Sales"].sum()
    by_dept = sales_df.groupby("Department")["Sales"].sum()
    by_month = sales_df.groupby(sales_df["Date"].dt.month)["Sales"].sum()
...
def analyze_sales_correlations():
    ...
    nums = merged.select_dtypes(include=[np.number])
    store_correlations = nums.corr()

    top = store_correlations["AnnualSales"].drop("AnnualSales")
    top = top.reindex(top.abs().sort_values(ascending=False).index)
    top_correlations = list(top.items())
...
def compare_store_performance():
    ...
    performance_ranking.plot(kind="bar")
    plt.title("Store Profit Ranking")
    plt.xlabel("Store")
...
def analyze_seasonal_patterns():
    ...
    monthly_sales.plot(kind="line", marker="o", label="monthly")
    dow_sales.plot(kind="line", marker="o", label="day of week")
...
def predict_store_sales():
    ...
    x1 = model_df["SquareFootage"].values
    x2 = model_df["StaffCount"].values
    x3 = model_df["YearsOpen"].values
    x4 = model_df["WeeklyMarketingSpend"].values
    y = model_df["AnnualSales"].values

    X = np.column_stack((np.ones(len(model_df)), x1, x2, x3, x4))
    ...
    coefficients = {
        "SquareFootage": float(betas[1]),
        "StaffCount": float(betas[2]),
        "YearsOpen": float(betas[3]),
        "WeeklyMarketingSpend": float(betas[4])
    }

    predictions = pd.Series(preds, index=model_df["Store"])
...
def forecast_department_sales():
    ...
    dept_trends.plot(ax=plt.gca(), marker="o")
    plt.title("Dept Sales Trends")
    plt.xlabel("Month")
    plt.ylabel("Sales")
    plt.tight_layout()
...
def identify_profit_opportunities():
    ...
    """
    combo = sales_df.groupby(["Store", "Department"])[["Sales", "Profit"]].sum()
    combo["ProfitMargin"] = combo["Profit"] / combo["Sales"]

    top_combinations = combo.sort_values("Profit", ascending=False).head(10).reset_index()
    underperforming = combo.sort_values("Profit", ascending=True).head(10).reset_index()
...
def develop_recommendations():
    ...
    """
    recommendations = [
        "Put more marketing into the stores that already sell the most.",
        "Keep pushing Prepared Foods and Bakery since their margins are stronger.",
        "Look at Jacksonville and Gainesville for efficiency problems.",
        "Target Family Shopper and Gourmet Cook customers with better promos.",
        "Plan staffing and inventory around busy months like summer and December."
    ]
...
def generate_executive_summary():
    ...
    print("Overview:")
    print("This analysis looked at GreenGrocer sales, customers, and store data for the year. The goal was to see which stores and departments did best, what patterns showed up in the data, and what the company could do to improve results.")
    print()

    print("Key Findings:")
    print(f"- {top_store} had the highest total sales.")
    print(f"- {top_profit_store} had the highest annual profit.")
    print(f"- {top_dept} was the top department for sales.")
    print(f"- {top_segment} customers spent the most on average per month.")
    print(f"- Month {best_month} had the highest monthly sales.")
    print()

    print("Recommendations:")
    print("- Put more effort into high-performing stores.")
    print("- Keep focusing on departments with better profit margins.")
    print("- Improve weaker stores by checking staffing and efficiency.")
    print("- Use customer segment info for better promotions.")
    print("- Get ready for busy months with more inventory and labor.")
    print()

    print("Expected Impact:")
    print("If GreenGrocer focuses more on the strongest stores, better departments, and higher-spending customers, the company should be able to improve profits and use resources better across locations.")
